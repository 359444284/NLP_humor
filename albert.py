...
class MyModel(nn.Module):
    def __init__(self, freeze_bert=False):
        ...
        self.tower_1 = nn.Sequential(
            nn.Dropout(p=0.8),
            nn.Linear(self.model.config.hidden_size, self.model.config.hidden_size/2.0),
            nn.ReLU(),
            nn.Linear(self.model.config.hidden_size/2.0, 2),
            nn.Softmax(dim=1)
        )
        ...
        self.tower_2 = nn.Sequential(
            ...
            nn.Linear(self.model.config.hidden_size, self.model.config.hidden_size/2.0),
            nn.ReLU(),
            nn.Linear(self.model.config.hidden_size/2.0, 1)
        )
